...
class ExecStartRequest(object):
    ...
    def __init__(self, service, scheme, endpoint, method, region_id, container_id, exec_id, pod_id=None):
...
def exec_start(app, service, region_id, container_id, exec_id, pod_id=None):
    def handle_signal(signum, frame):
        h, w = web_socket.get_win_size()
        resize_tty(h, w, app, service, region_id, container_id, exec_id)
    ...
    request = ExecStartRequest(service, WEBSOCKET_SCHEME, cli_config.endpoint, METHOD_GET, region_id, container_id, exec_id, pod_id=pod_id)
    request.invoke_shell(credential)

    h_o, w_o = web_socket.get_win_size()
    resize_tty(h_o, w_o, app, service, region_id, container_id, exec_id, pod_id=pod_id)
